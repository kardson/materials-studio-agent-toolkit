...
# User-facing GUI-resident dispatcher entrypoint.
# Run this script manually from Materials Studio GUI on Client with any document.
#
# This root-level file is the stable entrypoint users should look for.
# When the loop implementation is updated, update this file in place so
# `gui_loop.pl` always means "the current recommended GUI loop".
...
append_text($started_marker, "GUI loop started at " . scalar(localtime()) . "\n");
print "GUI loop started\n";
print "Queue: $queue_root\n";
write_status_line("started", "gui_loop", $queue_root);
...
while (1) {
    ...
    if ($now - $last_heartbeat >= 30) {
        write_status_line("heartbeat", "gui_loop", $queue_root);
        $last_heartbeat = $now;
    }
...
write_status_line("stopped", "gui_loop", "");
print "GUI loop stopped\n";
